...
- **Don't:** no `rounded-*` (even `rounded-full` → `rounded-none` via `fix.py`), no `shadow-lg/blur/backdrop-blur` (removed from `Results` modal), no `border-gray-*`, no `bg-tertiary` CTA (replaced with white/yellow/black), no non-mono stat values (use `font-mono`), no TypeWithMe copy, no emoji, no mouse-only flows (keep keyboard focus `ring-[3px] shadow-[4px_4px_0_0_#ffd400]`). Exception: `Auth.tsx` `AbstractPanel` uses subtle `radial-gradient #ffd400` + `linear-gradient` grid (`opacity 0.06/0.30`, `blur-3xl`) — intentional for branded depth, not page blur.
