...
def test_create_pipeline_returns_201(client: TestClient) -> None:
    ...
    body = {
        ...
        "name": "Test Pipeline",
        "encoding": {"strategy": "one_hot"},
        "scaling": {"strategy": "standard"},
        "split": {"test_size": 0.2, "random_seed": 42},
    }
...
def test_create_pipeline_no_target_column_returns_422(client: TestClient) -> None:
    ds_id = _upload_csv(client)
    response = client.post("/api/v1/pipelines/", json={"dataset_id": ds_id})
...
def test_list_pipelines_returns_paginated(client: TestClient) -> None:
    ds_id = _upload_csv(client)
    client.post("/api/v1/pipelines/", json={"dataset_id": ds_id, "target_column": "target"})
...
def test_get_pipeline_returns_pipeline(client: TestClient) -> None:
    ds_id = _upload_csv(client)
    create = client.post("/api/v1/pipelines/", json={"dataset_id": ds_id, "target_column": "target"})
...
def test_update_pipeline_returns_updated(client: TestClient) -> None:
    ...
    create = client.post("/api/v1/pipelines/", json={"dataset_id": ds_id, "target_column": "target"})
    pipe_id = create.json()["id"]
    response = client.put(f"/api/v1/pipelines/{pipe_id}", json={"name": "Updated Name", "target_column": "target"})
    assert response.status_code == 200
    assert response.json()["name"] == "Updated Name"


def test_update_nonexistent_pipeline_returns_404(client: TestClient) -> None:
    response = client.put(f"/api/v1/pipelines/{uuid.uuid4()}", json={"name": "Nope", "target_column": "target"})
    assert response.status_code == 404
